   ...
   # TODO: Add direction code for servos      
    ...
    for x in range(maxStep):
        ...
        if(twoStep> x):
            ...
            sleep(.001)            
def singleKinematics(target_position):
...
def checkError(position, angles):
    ...
    actPosition = computePosition[:3,3]
    xGood = abs(actPosition[0] - position[0]) < error
    yGood = abs(actPosition[1] - position[1]) < error
    zGood = abs(actPosition[2] - position[2]) < error
    print("{:.10f}".format(abs(actPosition[0]-position[0])))
    ...
    if(not xGood):
        print("c")
        errorMessage += " x value is inaccurate by " + str(abs(position[0]-actPosition[0]))
    if(not yGood):
        print("d")
        errorMessage += " y value is inaccurate by " + str(abs(position[1]-actPosition[1]))
    if(not zGood):
        print("e")
        errorMessage += " z value is inaccurate by " + str(abs(position[2]-actPosition[2]))
    print(errorMessage)
...
def runTxtCommands(conn):
    while True:
        ...
        replyNums = "Received " + str(x) + " " + str(y) + " " + str(z) + " starting motion"
        print(replyNums)
        conn.sendall(str.encode(replyNums))
        print("moving")
...
def runAngleCommands(conn):
    while True:
        ...
        print(end)
        print(c)
        replyNums = "Received " + str(b) + " " + str(a1) + " " + str(a2) + " " + str(spin) + " " + str(end) + " starting motion"
        print(replyNums)
        conn.sendall(str.encode(replyNums))
        print("moving")
        moveAll(b, a1, a2, spin, end)
        claw(c)
        print("moved")
        break
    conn.close()
